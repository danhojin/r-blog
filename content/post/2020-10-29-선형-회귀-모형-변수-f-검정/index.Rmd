...
자유도는 특징 변수의 축소는 $\Delta df = df_R - df_F$ 자유도의 차이이다. 이를 이용하면 다음 F비가 정의된다.
...
선형 다중 회귀에서 변수 결정에 유용하게 쓸 수 있는 F-검정을 살펴보았다. 이 논의에서 얻을 수 있는 잔차 합과 자유도의 관계가 모형 비교 기준을 만드는데 어떻게 쓰였는지 lasso에 어떻게 확장되었는지 확인해 보았다. lasso는 변수 결정에 매우 중요한 방법이므로 차후에 다시 살펴보도록 하겠다.
