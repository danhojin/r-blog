---
title: 교호 작용 선택을 위한 라쏘 회귀 - glinternet 소개
author: 단호진
date: '2020-11-14'
slug: 교호-작용-라쏘-회귀-glinternet-소개
categories: []
tags:
  - lasso
  - interaction effects
...
coefs <- coef(cvfit$glinternetFit)[[index_iStd]]
coefs$mainEffects
```
...
이 예제에서는 분산-편향성 절출 관계가 명확하게 드러나지 않는다. 산점도나 CV error 그래프에도 확인할 수 있듯 데이터의 분산이 크기 때문일 것이다. 여전히 glinternet은 lambda 값을 lamdaHat1Std에 저장하여 사용자에게 제안해 준다. 이 예제의 경우 lambda의 범위 내에서 CV error가 가장 적은 마지막 값이 되었다. 그 lambda에서 결과를 coefs에 저장하였다. 교호 효과 interactions에서 contcont(연속형 특징 변수 간 교호작용) educ(1)과 exper(2)j의 교호 작용이 존재하고, 주 효과 mainEffects에 1, 2이 모두 나타나 있다. 각 계수 추정치는 다음과 같다.
...
특징 변수가 많아지면 교호 효과를 평가해 보는 것이 쉽지 않다. 하지만 glinternet 패키지의 활용하면 특징 변수 평가 선정 과정을 보다 손쉽게 수행할 수 있다는 점을 확인하였다. 
